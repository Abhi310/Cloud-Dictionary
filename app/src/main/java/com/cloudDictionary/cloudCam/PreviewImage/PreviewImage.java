...
public class PreviewImage extends Activity {
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if(e1.getX() - e2.getX() > SWIPE_MINIMUM_DISTANCE && Math.abs(velocityX) > SWIPE_THRESHOLD_VELOCITY) {

                File nextPicture = BucketFiles.getNextPictureFile(currFile);

                if(nextPicture != null && BucketFiles.isFileExists(nextPicture)) {

                    currFile = nextPicture;
            ...
            }  else if (e2.getX() - e1.getX() > SWIPE_MINIMUM_DISTANCE && Math.abs(velocityX) > SWIPE_THRESHOLD_VELOCITY) {

                File previusPicture = BucketFiles.getPreviusPictureFile(currFile);

                if(previusPicture != null && BucketFiles.isFileExists(previusPicture)) {

                    currFile = previusPicture;
            ...
            if(e1.getY() - e2.getY() > SWIPE_MINIMUM_DISTANCE && Math.abs(velocityY) > SWIPE_THRESHOLD_VELOCITY) {
                return false; // Bottom to top
            }  else if (e2.getY() - e1.getY() > SWIPE_MINIMUM_DISTANCE && Math.abs(velocityY) > SWIPE_THRESHOLD_VELOCITY) {
    ...
    public void share(View v) {
        ...
        intent.putExtra(Intent.EXTRA_STREAM, currFile);
